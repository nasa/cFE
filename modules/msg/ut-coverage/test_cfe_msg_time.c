...
void Test_MSG_Time(void)
{
    ...
    CFE_UtAssert_EQUAL(CFE_MSG_GetMsgTime(NULL, &actual), CFE_MSG_BAD_ARGUMENT);
    CFE_UtAssert_EQUAL(actual.Seconds, 0xFFFFFFFF);
    CFE_UtAssert_EQUAL(actual.Subseconds, 0xFFFFFFFF);
    CFE_UtAssert_EQUAL(CFE_MSG_GetMsgTime(msgptr, NULL), CFE_MSG_BAD_ARGUMENT);
    CFE_UtAssert_EQUAL(Test_MSG_NotZero(msgptr), 0);
    CFE_UtAssert_EQUAL(CFE_MSG_SetMsgTime(NULL, input[0]), CFE_MSG_BAD_ARGUMENT);
    CFE_UtAssert_EQUAL(CFE_MSG_SetMsgTime(msgptr, actual), CFE_MSG_WRONG_MSG_TYPE);
    CFE_UtAssert_EQUAL(Test_MSG_NotZero(msgptr), 0);
    CFE_UtAssert_EQUAL(CFE_MSG_GetMsgTime(msgptr, &actual), CFE_MSG_WRONG_MSG_TYPE);
    CFE_UtAssert_EQUAL(actual.Seconds, 0);
    CFE_UtAssert_EQUAL(actual.Subseconds, 0);
    ...
    for (i = 0; i < sizeof(input) / sizeof(input[0]); i++)
    {
        ...
        CFE_UtAssert_EQUAL(CFE_MSG_GetMsgTime(msgptr, &actual), CFE_SUCCESS);
        CFE_UtAssert_EQUAL(actual.Seconds, 0xFFFFFFFF);
        CFE_UtAssert_EQUAL(actual.Subseconds, 0xFFFF0000);
        CFE_UtAssert_EQUAL(CFE_MSG_SetMsgTime(msgptr, input[i]), CFE_SUCCESS);
        UT_DisplayPkt(msgptr, sizeof(tlm));
        CFE_UtAssert_EQUAL(CFE_MSG_GetMsgTime(msgptr, &actual), CFE_SUCCESS);
        CFE_UtAssert_EQUAL(actual.Seconds, input[i].Seconds);
        CFE_UtAssert_EQUAL(actual.Subseconds, input[i].Subseconds & 0xFFFF0000);
        CFE_UtAssert_EQUAL(Test_MSG_NotF(msgptr), MSG_TYPE_FLAG);
    }
