...
void UtTest_Setup(void)
{
    ...
    Test_TA_Cmds();
    Test_TA_QueryAll();
    Test_TA_StartupAffinity();
    Test_TA_Events();
}

void Test_TA_Init(void)
{
...
void Test_TA_StartupAffinity(void)
{
...
{
    /* CFE_Config_GetObjPointer defaults to returning NULL, providing perfect test coverage of the fallback */
...
/* Helper to perfectly test Bad Length handling by letting the default framework read real headers */
static void TA_UT_SetupBadLengthCmd(CFE_SB_Buffer_t *BufPtr, CFE_MSG_FcnCode_t FcnCode, CFE_MSG_Size_t RealSize)
{
    memset(BufPtr, 0, sizeof(*BufPtr));

    /* Initialize the raw packet so the native default stubs can parse it naturally */
    CFE_MSG_Init(&BufPtr->Msg, CFE_SB_ValueToMsgId(CFE_TA_CMD_MID), RealSize + 10); /* Force invalid length */
    CFE_MSG_SetFcnCode(&BufPtr->Msg, FcnCode);

    /* Safely force EDS dispatcher to fail if we are in an EDS build */
    if (EdsDispatch_EdsComponent_CFE_TA_Application_Telecommand != NULL)
    {
...
void Test_TA_TaskPipe_InvalidMsgId(void)
{
    ...
    UT_CallTaskPipe(CFE_TA_TaskPipe, &Buf.Msg, sizeof(Buf), UT_TPID_CFE_TA_INVALID_MID);
}
...
void Test_TA_ProcessCmd_BadCC(void)
{
    CFE_SB_Buffer_t Buf;
    memset(&Buf, 0, sizeof(Buf));
    UT_CallTaskPipe(CFE_TA_TaskPipe, &Buf.Msg, sizeof(Buf), UT_TPID_CFE_TA_CMD_INVALID_CC);
}

void Test_TA_ProcessCmd_BadLength_Noop(void)
{
    CFE_SB_Buffer_t Buf;
    TA_UT_SetupBadLengthCmd(&Buf, CFE_TA_NOOP_CC, sizeof(CFE_TA_NoopCmd_t));
    CFE_TA_TaskPipe(&Buf);
...
{
    CFE_SB_Buffer_t Buf;
    TA_UT_SetupBadLengthCmd(&Buf, CFE_TA_RESET_COUNTERS_CC, sizeof(CFE_TA_ResetCountersCmd_t));
    CFE_TA_TaskPipe(&Buf);
    UtAssert_INT32_EQ(CFE_TA_Global.CommandErrorCounter, 1);
    UtAssert_STUB_COUNT(CFE_EVS_SendEvent, 1);
}
...
{
    CFE_SB_Buffer_t Buf;
    TA_UT_SetupBadLengthCmd(&Buf, CFE_TA_QUERY_ALL_TASKS_CC, sizeof(CFE_TA_QueryAllTasksCmd_t));
    CFE_TA_TaskPipe(&Buf);
    UtAssert_INT32_EQ(CFE_TA_Global.CommandErrorCounter, 1);
    UtAssert_STUB_COUNT(CFE_EVS_SendEvent, 1);
}
...
{
    CFE_SB_Buffer_t Buf;
    TA_UT_SetupBadLengthCmd(&Buf, CFE_TA_SET_TASK_AFFINITY_CC, sizeof(CFE_TA_SetTaskAffinityCmd_t));
    CFE_TA_TaskPipe(&Buf);
    UtAssert_INT32_EQ(CFE_TA_Global.CommandErrorCounter, 1);
    UtAssert_STUB_COUNT(CFE_EVS_SendEvent, 1);
}
...
{
    CFE_SB_Buffer_t Buf;
    TA_UT_SetupBadLengthCmd(&Buf, CFE_TA_GET_TASK_AFFINITY_CC, sizeof(CFE_TA_GetTaskAffinityCmd_t));
    CFE_TA_TaskPipe(&Buf);
    UtAssert_INT32_EQ(CFE_TA_Global.CommandErrorCounter, 1);
    UtAssert_STUB_COUNT(CFE_EVS_SendEvent, 1);
}
