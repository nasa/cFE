...
void Test_TransmitTxn_Init(void)
{
    ...
    UtAssert_BOOL_FALSE(Txn->UpdateVerifyReq);
}
...
void UT_CFE_MSG_Verify_CustomHandler(void *UserObj, UT_EntryKey_t FuncKey, const UT_StubContext_t *Context)
{
    ...
    *VerifyStatus = (UT_GetStubCount(FuncKey) & 1) == 0;
}
...
void Test_ReceiveBufferWithRoute(void)
{
    ...
    UT_SetHandlerFunction(UT_KEY(CFE_MSG_Verify), UT_CFE_MSG_Verify_CustomHandler, NULL);
    CFE_UtAssert_SETUP(CFE_SB_CreatePipe(&PipeId, 3, "TestPipe"));
    ...
    UtAssert_STUB_COUNT(CFE_MSG_Verify, 0);
    ...
    UtAssert_STUB_COUNT(CFE_MSG_Verify, 2);
    ...
    UtAssert_STUB_COUNT(CFE_MSG_Verify, 3);
