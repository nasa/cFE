...
{
    TxnPtr->UpdateVerifyReq = EnableUpdate;
}
...
void CFE_SB_TransmitTxn_FindDestinations(CFE_SB_MessageTxn_State_t *TxnPtr, CFE_SB_BufferD_t *BufDscPtr)
{
    ...
    if (CFE_SBR_IsValidRouteId(BufDscPtr->DestRouteId))
    {
        ...
        if (TxnPtr->UpdateVerifyReq)
        {
            CFE_SBR_IncrementSequenceCounter(BufDscPtr->DestRouteId);

            /* Update all MSG headers based on the current sequence */
            CFE_MSG_UpdateHeader(&BufDscPtr->Content.Msg, CFE_SBR_GetSequenceCounter(BufDscPtr->DestRouteId),
                                 TxnPtr->ContentSize);

            /* Clear the flag, just in case */
            TxnPtr->UpdateVerifyReq = false;
        }
...
{
    TxnPtr->UpdateVerifyReq = EnableVerify;
}
...
const CFE_SB_Buffer_t *CFE_SB_ReceiveTxn_Execute(CFE_SB_MessageTxn_State_t *TxnPtr)
{
    ...
    while (CFE_SB_MessageTxn_IsOK(TxnPtr))
    {
        ...
        if (TxnPtr->UpdateVerifyReq)
        {
            CFE_MSG_Verify(&BufDscPtr->Content.Msg, &VerifyStatus);
        }
        else
        {
            /* If no verification being done, consider everything "good" */
            VerifyStatus = true;
        }
        ...
        CFE_SB_MessageTxn_ReportSingleEvent(TxnPtr, TxnPtr->PipeSet, CFE_SB_RCV_MESSAGE_VALIDATION_FAIL_EID);
