...
CFE_Status_t CFE_SB_ReceiveBuffer(CFE_SB_Buffer_t **BufPtr, CFE_SB_PipeId_t PipeId, int32 TimeOut)
{
    ...
    if (CFE_SB_MessageTxn_IsOK(Txn))
    {
         ...
         * Use CFE_SB_ReceiveBufferWithRoute() to skip verification if needed.
         */
        CFE_SB_ReceiveTxn_SetVerify(Txn, true);
    }
 ...
 *
 *-----------------------------------------------------------------*/
CFE_Status_t CFE_SB_ReceiveBufferWithRoute(CFE_SB_PipeId_t PipeId, const CFE_SB_Buffer_t **BufPtr,
                                           CFE_MSG_Size_t *BufSize, CFE_SB_MsgId_t *RoutingMsgId, bool EnableVerify,
                                           int32 TimeOut)
{
    CFE_SB_ReceiveTxn_State_t  TxnBuf;
    CFE_SB_MessageTxn_State_t *Txn;
    ...
    {
        CFE_SB_ReceiveTxn_SetPipeId(Txn, PipeId);
        CFE_SB_ReceiveTxn_SetVerify(Txn, EnableVerify);
    }
...
CFE_Status_t CFE_SB_TransmitBuffer(CFE_SB_Buffer_t *BufPtr, bool EnableUpdate)
{
    ...
    if (CFE_SB_MessageTxn_IsOK(Txn))
    {
        ...
        CFE_SB_TransmitTxn_SetUpdate(Txn, EnableUpdate);
