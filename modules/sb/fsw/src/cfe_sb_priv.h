 ...
 */
bool CFE_SB_MessageTxn_IsOK(const CFE_SB_MessageTxn_State_t *TxnPtr);
...
/*---------------------------------------------------------------------------------------*/
/**
 * \brief Configure message verification during a receive transaction
 *
 * Enables message verification during the execution of a receive transaction.  If configured
 * to "true", then the CFE_MSG_Verify() API will be invoked before the message buffer
 * is returned to the caller.  If this fails, then an event will be logged, the message will
 * be dropped, and the implementation will get the next message in the queue (if any).
 *
 * \param[inout] TxnPtr         Transaction object
 * \param[in]    EnableVerify   Pass as "true" to enable verification during execution
 */
void CFE_SB_ReceiveTxn_SetVerify(CFE_SB_MessageTxn_State_t *TxnPtr, bool EnableVerify);
...
/*---------------------------------------------------------------------------------------*/
/**
 * \brief Configure header updates during a transmit transaction
 *
 * Enables header updates during the execution of a transmit transaction.  If configured
 * to "true", then the CFE_MSG_UpdateHeader() API will be invoked before the message buffer
 * is broadcast to subscribers.
 *
 * \note Some of the information that is included in a header update comes from the Route -
 * and thus needs to be done synchronously with message routing (that is, the header update
 * cannot be performed before message routing is performed).  That is why the header update,
 * if requested, needs be done as part of the message routing and not prior to it.
 *
 * \param[inout] TxnPtr         Transaction object
 * \param[in]    EnableUpdate   Pass as "true" to enable header update during execution
 */
void CFE_SB_TransmitTxn_SetUpdate(CFE_SB_MessageTxn_State_t *TxnPtr, bool EnableUpdate);
