...
void OS_printf(const char *string, ...)
{
    ...
    else
    {
        ...
        if ((start = strchr(tmpString, dash)))
        {
            ...
            else if (strncmp(start, "ES Startup: Could not load cFE Shared "
                               "Library: ", 46) == 0)
            {
                OSPrintRtn.value += UT_OSP_LOAD_SHARED_LIBRARY;
            }
            else if (strncmp(start, "ES Startup: Could not find Library Init symbol", 46) == 0)
            {
            ...
            else if (UT_strcmp(start, "ES Startup: Failed to unload APP: "
                               "AppName. EC = 0x~\n") == 0)
            {
                OSPrintRtn.value += UT_OSP_MODULE_UNLOAD_FAILED;
            }
            else if (strncmp(start, "ES Startup: Unable to extract filename "
                             "from path: ut57/", 55) == 0)
            {
            ...
            {
                OSPrintRtn.value += UT_OSP_STARTUP_SYNC_FAIL_1;
            }
            else if (UT_strcmp(start, "ES Startup: Startup Sync failed - "
                               "Applications may not have all initialized\n") == 0)
            {
                OSPrintRtn.value += UT_OSP_STARTUP_SYNC_FAIL_2;
            }
