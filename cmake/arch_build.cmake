...
#
function(add_cfe_coverage_test MODULE_NAME)

    cmake_parse_arguments(ADDCOVTEST_ARG "ALL" "UNIT_NAME" "TESTCASE_SOURCES;SUBJECT_SOURCES" ${ARGN})

    # If not specified by keyword, then the unit name is the first positional argument (backward compatible)
    if (NOT ADDCOVTEST_ARG_UNIT_NAME)
      # If "ALL" keyword was specified, this becomes the unit name (again for backward compatibility)
      if (ADDCOVTEST_ARG_ALL)
        set(ADDCOVTEST_ARG_UNIT_NAME "ALL")
      else(ADDCOVTEST_ARG_ALL)
        list(GET ADDCOVTEST_ARG_UNPARSED_ARGUMENTS 0 ADDCOVTEST_ARG_UNIT_NAME)
        list(REMOVE_AT ADDCOVTEST_ARG_UNPARSED_ARGUMENTS 0)
      endif(ADDCOVTEST_ARG_ALL)
    ...
    if (NOT ADDCOVTEST_ARG_TESTCASE_SOURCES)
      ...
      list(REMOVE_AT ADDCOVTEST_ARG_UNPARSED_ARGUMENTS 0)
    endif(NOT ADDCOVTEST_ARG_TESTCASE_SOURCES)

    # If not specified by keyword, then all remaining positional arguments are considered test subject source files
    if (NOT ADDCOVTEST_ARG_SUBJECT_SOURCES)
      set(ADDCOVTEST_ARG_SUBJECT_SOURCES ${ADDCOVTEST_ARG_UNPARSED_ARGUMENTS})
    endif(NOT ADDCOVTEST_ARG_SUBJECT_SOURCES)

    # Another small thing to avoid a common nuisance/annoyance when using relative paths
    # The list of sources often comes from the FSW module and is relative to the module top level,
    # but this function is usually invoked from a "ut-coverage" directory, and that breaks the paths.
    set(SCRUBBED_SUBJECT_SOURCES)
    foreach(SUBJECT_SRC ${ADDCOVTEST_ARG_SUBJECT_SOURCES})
      if (IS_ABSOLUTE "${SUBJECT_SRC}" OR EXISTS "${SUBJECT_SRC}")
        # Absolute paths are unambiguous already - always use these directly as is.
        # OR - It is a relative path but it looks valid from this directory - also use it as is.
        # Do nothing in this case - just pass through.
      elseif(EXISTS "${PROJECT_SOURCE_DIR}/${SUBJECT_SRC}")
        # The most common use case is when a list of relative paths was assembled in the project dir,
        # but then used in a coverage testing subdirectory.  Adjust the path now to point to it.
        # Keep it as a relative path though -- this results in a simpler build tree than absolute paths
        file(RELATIVE_PATH SUBJECT_SRC ${CMAKE_CURRENT_SOURCE_DIR} "${PROJECT_SOURCE_DIR}/${SUBJECT_SRC}")
      endif()

      # Now add the scrubbed path to the list that will be used to create the UT subjects target
      list(APPEND SCRUBBED_SUBJECT_SOURCES "${SUBJECT_SRC}")

    endforeach()


    # A consistent name convention for all targets generated by this function
    set(TEST_NAME "coverage-${MODULE_NAME}-${ADDCOVTEST_ARG_UNIT_NAME}")
    ...
    add_executable(${RUNNER_TARGET}
        ${ADDCOVTEST_ARG_TESTCASE_SOURCES}
    ...
    if (MODULE_DEPENDENCIES)
        add_cfe_coverage_dependency(${MODULE_NAME} ${ADDCOVTEST_ARG_UNIT_NAME} ${MODULE_DEPENDENCIES})
